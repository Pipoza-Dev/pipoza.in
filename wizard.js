...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const stepLabels = {
    1: "Project Category",
    2: "Business & Vision",
    3: "Style & Theme Direction",
    4: "Contact & Budget",
    5: "Review & Instant Launch"
  };
  ...
  if (typeGrid) {
    const typeOptions = [...PIPOZA.CATEGORIES, { id:'other', label:'Something Else', desc:'Custom bespoke project scope' }];
    typeGrid.innerHTML = typeOptions.map(c => `
      <div class="type-opt tilt-card" data-id="${c.id}">
  ...
  if (presetsBar) {
    ...
    presetsBar.addEventListener('click', e => {
      ...
      if (typeof updateBudgetUI === 'function') {
        let presetVal = 35000;
        if (p.budget.includes('10k – 25k')) presetVal = 18000;
        if (p.budget.includes('25k – 50k')) presetVal = 35000;
        if (p.budget.includes('Under')) presetVal = 8000;
        if (p.budget.includes('50k')) presetVal = 65000;
  ...
  const STYLES = ['Minimal & clean', 'Bold & high-energy', 'Corporate & trustworthy', 'Futuristic cyberpunk', 'You decide — artist choice'];
  const styleGrid = document.getElementById('styleGrid');
  if (styleGrid) {
    styleGrid.innerHTML = STYLES.map(s => `<div class="chip-opt" data-s="${s}">${s}</div>`).join('');
  ...
  function getBudgetTierInfo(val) {
    if (val < 10000) {
      return { tier: 'Under ₹10k', desc: 'Essential Landing', label: `₹${val.toLocaleString('en-IN')}` };
    } else if (val <= 25000) {
      return { tier: '₹10k – 25k', desc: 'Standard 4–5 Day Sprint', label: `₹${val.toLocaleString('en-IN')}` };
    } else if (val <= 50000) {
      return { tier: '₹25k – 50k', desc: 'SaaS, Clinics & Stores', label: `₹${val.toLocaleString('en-IN')}` };
    } else if (val < 100000) {
      return { tier: '₹50k – 100k', desc: 'Advanced Web Applications', label: `₹${val.toLocaleString('en-IN')}` };
    } else {
      return { tier: '₹100k+', desc: 'Full Custom Architecture', label: '₹1,00,000+' };
    }
  }
  ...
  function updateBudgetUI(val, playSound = false) {
    ...
    const min = +budgetSlider.min || 8000;
    const max = +budgetSlider.max || 100000;
  ...
  if (budgetSlider) {
    updateBudgetUI(+budgetSlider.value || 25000, false);
    ...
    if (btnFlexibleQuote) {
      btnFlexibleQuote.addEventListener('click', () => {
        ...
        if (isCustomQuote) {
          if (budgetAmount) budgetAmount.textContent = 'Custom Scope';
  ...
  if (nextBtn) nextBtn.addEventListener('click', () => goTo(step + 1));
  ...
  function gatherData() {
    ...
    return {
      type: selectedType ? (selectedType.id === 'other' && otherType ? otherType : selectedType.label) : 'Tech & Startups',
  ...
  function buildReview() {
    const d = gatherData();
    const rows = [
      ['Website Category', d.type],
      ['Business Name', d.bizName],
      ['Location / Base', d.bizLocation],
      ['Scope & Vision', d.bizDesc],
      ['Style Preference', d.style],
      ['Color Palette', d.palette],
      ['Contact Person', d.ctName],
      ['Email Address', d.ctEmail],
      ['Phone / WhatsApp', d.ctPhone],
      ['Target Budget', d.budget],
      ['Delivery Estimate', '4–5 working days (Free Hosting included)']
    ];
  ...
  function buildMessage(d) {
    return [
      ...
      `Budget: ${d.budget}`,
      'Hosting: Free PipoZa Hosting Included',
